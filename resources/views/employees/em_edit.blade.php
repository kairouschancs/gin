...
@section('content')
    ...
    <div class="row">
        <div class="border col-7">
            <form action="{{ route('employees.em_update', $employee->id) }}" method="post">
                ...
                @csrf 
                        <div class="col-sd">
                            <div class="row">
                                ...
                                <div class="form-group col-md-4">
                                    ...
                                    <input type="text" class="form-control" id="employee_code" name="employee_code" value="{{ old('employee_code',$employee->employee_code) }}"
                                ...
                                <div class="form-group col-md-4">
                                    ...
                                    <select class="form-control" id="role_id" name="role_id">
                                        @foreach ($roles as $role)
                                            <option value="{{ $role->id }}" @if( old('role', $employee->role) == $role->role) selected @endif>{{ $role->role }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                {{-- 従業員名入力 --}}
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="text" class="form-control" id="holiday_calendar_type" name="holiday_calendar_type" value="{{ old('holiday_calendar_type',$employee->holiday_calendar_type) }}"
                                    placeholder="休日カレンダーType を入力" />
                                </div>
                                {{-- 退職 -> 退職 = 1 --}}
                                <div class="form-group col-md-6">
                                    <label for="retirement">在籍Status<span class="badge badge-secondary ml-2">{{ __('必須') }}</span></label>
                                    <input type="text" class="form-control" id="retirement" name="retirement" value="{{ old('retirement',$employee->retirement) }}"
                    ...
                    <div class="row center-block text-center">
                        ...
                        <div class="col-6">
                            {{-- 編集画面に遷移 --}}
                            <button type="button" class="btn btn-outline-secondary btn-block" onclick="location.href='{{ route('employees.index') }}'">戻る</button>
                        </div>
                        <div class="col-6">
